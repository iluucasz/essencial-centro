...
import { enviarEmailDeTeste, type EstadoTesteWhatsApp } from "@/modules/notificacoes/actions";
...
export function FormularioTesteEmail() {
  const [state, formAction, pending] = useActionState(enviarEmailDeTeste, estadoInicial);

  return (
    <form action={formAction} className="grid min-w-0 gap-5">
      <div className="grid min-w-0 gap-2">
        <label className="text-sm font-medium text-foreground" htmlFor="email-teste">
          E-mail de destino
        </label>
        <input
          ...
          id="email-teste"
          name="email"
          placeholder="Ex.: pessoa@exemplo.com"
          required
          type="email"
        />
      </div>
      ...
      <div className="grid min-w-0 gap-2">
        <label className="text-sm font-medium text-foreground" htmlFor="mensagem-teste">
          Mensagem de teste
        </label>
        <textarea
