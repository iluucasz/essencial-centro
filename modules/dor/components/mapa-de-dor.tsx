...
export function MapaDeDor({
  ...
  /**
   * `onOpenChange` limpa a seleção em QUALQUER fechamento — Esc, clique no backdrop e o X do
   * cabeçalho não passam pelo nosso botão Cancelar, e sem isso o badge continuaria destacado no
   * modelo depois de fechar.
   */
  ...
  /** Estado atual por âncora — alimenta a legenda, os badges e o valor inicial do modal. */
  const registradoPorAncora = new Map(
    // `pontos` vem do mais recente pro mais antigo: invertendo, o último a escrever é o atual.
    [...pontos].reverse().map((ponto) => [`${chaveAncora(ponto)}:${ponto.anterior}`, ponto]),
  );

  /**
   * Abre o modal já com a intensidade do que houver gravado naquela âncora — reavaliar dor é o caso
   * comum entre sessões, e começar sempre em 5 obrigaria a reencontrar o valor anterior na lista.
   */
  function abrirSelecao(ancora: AncoraSelecionada) {
    const registrado = registradoPorAncora.get(`${chaveAncora(ancora)}:${ancora.anterior}`);

    setSelecao(ancora);
    setIntensidade(registrado?.intensidade ?? 5);
    modal.open();
  }

  /** Fechar basta: o `onOpenChange` acima é quem limpa seleção e slider. */
  ...
  const [estado, acao, enviando] = useActionState(
    async (anterior: EstadoRegistroDor, formData: FormData) => {
      ...
      if (resultado.status === "sucesso") fecharSelecao();
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      {/*
        Formulário em modal: escolher o ponto e definir a intensidade é uma ação focada, e no canto
        da tela ela competia com a lista de registros. O modal é CONTROLADO (aberto por código, sem
        `Modal.Trigger`) porque o gatilho é um badge dentro do canvas 3D.
      */}
      <Modal state={modal}>
        <Modal.Backdrop variant="opaque">
          <Modal.Container size="md">
            <ConteudoModal
              titulo={
                selecao
                  ? `${descreverRegiao(selecao.regiao, selecao.lado)} · ${selecao.anterior ? "frente" : "costas"}`
                  : "Registrar dor"
              }
            >
              {selecao ? (
                <form action={acao} className="grid gap-4">
                  ...
                  <label className="grid gap-1.5 text-sm">
                    ...
                    <textarea
                      ...
                      name="observacao"
                      placeholder={
                        comoCliente ? "Piora quando eu…" : "Palpação, irradiação, gatilho…"
                      }
                    />
                  </label>
      ...
      <div className="grid content-start gap-4">
        {estado.status === "sucesso" ? (
          <p className="rounded-2xl border border-brand/20 bg-brand/5 p-3 text-sm font-medium text-brand">
            {estado.mensagem}
          </p>
        ) : null}
