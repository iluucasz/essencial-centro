...
export function HeroSection() {
  return (
    <section id="inicio" className="relative overflow-hidden pt-16">
      <div className="pointer-events-none absolute -right-24 -top-24 h-96 w-96 rounded-full bg-sage/50 blur-3xl" />
      <div className="mx-auto grid max-w-6xl items-center gap-12 px-4 pb-20 pt-16 sm:px-6 lg:grid-cols-2 lg:gap-8 lg:px-8 lg:pt-24">
        <div className="relative z-10">
          <span className="inline-flex items-center gap-2 rounded-full border border-forest/20 bg-sage/60 px-4 py-1.5 text-xs font-medium uppercase tracking-[0.16em] text-forest-deep">
            <Star className="h-3.5 w-3.5" />
            Clínica de estética e bem-estar
          </span>

          <h1 className="mt-6 text-balance font-serif text-4xl font-semibold leading-[1.05] tracking-tight text-ink sm:text-5xl lg:text-6xl">
            Sua jornada de cuidado, do início aos{" "}
            <span className="text-forest">resultados</span>.
          </h1>

          <p className="mt-6 max-w-lg text-pretty text-lg leading-relaxed text-ink-soft">
            No Essencial Centro cada tratamento é acompanhado de perto: fichas
            digitais, evolução de medidas e um portal exclusivo para você
            visualizar seus resultados com segurança.
          </p>
        ...
        <div className="relative z-10">
          ...
          <div className="absolute -bottom-6 -left-6 hidden max-w-[15rem] rounded-2xl border border-line bg-cream/95 p-4 shadow-lg backdrop-blur sm:block">
            ...
            <p className="mt-1 text-xs leading-relaxed text-ink-soft">
              Medidas, fotos e evolução registradas a cada sessão no seu
              prontuário digital.
            </p>
          </div>
        </div>
