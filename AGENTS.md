...
1. **Antes de finalizar qualquer tarefa de código**: `pnpm check` (typecheck + lint + format + test) deve passar.
2. **Toda feature entra com teste** (Vitest) cobrindo a regra de negócio principal e a autorização.
3. **Formatação é do Prettier**, ordenação de imports/classes incluída — não formate à mão nem brigue com ele.
4. **Um pre-commit hook (Husky) roda lint-staged + typecheck e aborta o commit se falhar.** Não use
   `--no-verify`. Se o hook barrar, corrija a causa.
5. Nomes claros e em pt-BR para domínio; funções pequenas e coesas; sem código morto ou `console.log`.
6. Nunca commitar segredos, `.env.local` ou dados reais de pacientes.
