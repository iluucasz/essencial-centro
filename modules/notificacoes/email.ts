 ...
 * (`BREVO_API_KEY`/`BREVO_SENDER_EMAIL` ausentes) ou a chamada falhar, só registra o erro — o
 * canal in-app (`notificacao`) já cobriu a entrega, e-mail é um reforço, nunca bloqueante.
 */
export async function enviarEmailNotificacao(params: {
  ...
  if (!apiKey || !remetenteEmail) return;
